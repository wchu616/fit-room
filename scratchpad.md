...
- 修复计划 10:00 锁定计算，以用户时区当天 10:00 为基准
- 调整登录流：未登录访问根路径跳转登录，登录成功默认进入打卡页
- 导航顺序更新为房间/小队/计划/打卡/排名，并重命名统计面板
- 排名页（原统计）将小队排行榜上移至顶部
- 调整 app-input 字体颜色，提升如 "Asia/Shanghai" 等选择项的可读性
- 计划创建/编辑支持 UI 选择重复模式（每周、每 N 天、自定义 RRULE），替换原纯文本输入
